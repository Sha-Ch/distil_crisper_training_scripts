...
| `storage` | `min_free_space_gb`, `emergency_free_space_gb`, `chunk_size_gb` (overrides DATASET_CONFIGS chunk size), `flush_dataset_cache_after_completion` (small-disk: drop a dataset's HF cache after it completes, keeping the teacher) | cloud 50/20 + `chunk_size_gb: 100`; **local-4090** `config.local.yaml` 30/15 + `chunk_size_gb: 40` + `flush_dataset_cache_after_completion: true` for a ~160 GB local HF cache. Local-4090 splits storage: **HF cache on a local docker volume `/hf_cache`**, **outputs on a TrueNAS SMB share at `/workspace`** — see [LOCAL_4090.md](LOCAL_4090.md) |
...
- **Local-4090 storage split** (`config.local.yaml` + `docker-compose.yml`): **HF cache on a local docker volume** (`/hf_cache`, ext4) and **outputs on a TrueNAS SMB share** (`/workspace`), with `storage.flush_dataset_cache_after_completion: true` cycling the cache per dataset. Don't move the HF cache onto SMB — `datasets` mmap + hub-cache symlinks fight CIFS. iSCSI disks can't be bind-mounted into WSL2 (only physical fixed disks auto-mount), so `/workspace` uses an SMB share, not a drive path. SMB creds come from `.env` (`SMB_HOST/SMB_SHARE/SMB_USER/SMB_PASS`) — never commit them. See [LOCAL_4090.md](LOCAL_4090.md).
